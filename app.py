...
# Vercel specific: Use /tmp for writable directories
...
# Ensure the static/decrypted folder exists (mapped to tmp for vercel)
if os.environ.get('VERCEL'):
    DECRYPTED_FOLDER = '/tmp/decrypted'
else:
...
# Initialize DB on start (and on request for Vercel if missing)
...
def handle_encryption(allowed_extensions, redirect_route, type_name):
    ...
    if file and allowed_file(file.filename, allowed_extensions):
        ...
        try:
            # We need to monkeypatch encryption.py's load_key to use /tmp if on Vercel
            # Or better, update encryption.py. But for now let's assume encryption.py uses local dir.
            # If Vercel, we need to handle key storage. 
            # For this demo, we'll just let it try to write to current dir which might fail if not /tmp.
            # Actually, I should update encryption.py too.
